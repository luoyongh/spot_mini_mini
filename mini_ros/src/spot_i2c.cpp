/// \file
/// \brief 用i2c向系统提供如下部件的控制和数据：舵机（通过PCA9685），陀螺仪（MPU-6050），压力传感器

#include "SpotServo/SpotServo.hpp"
#include "ContactSensor/ContactSensor.hpp"
//#include <Kinematics.hpp> // 未使用
...
void command_servos(const LegJoints & legjoint, const bool & step_or_view = true)
{
  ...
  if (step_or_view)
  {
    ...
    s_speed = std::max(s_speed, viewing_speed / 10.0);
    e_speed = viewing_speed * e_dist;
    e_speed = std::max(e_speed, viewing_speed / 10.0);
    w_speed = viewing_speed * w_dist;
    w_speed = std::max(w_speed, viewing_speed / 10.0);
  } else
  {
...
// THIS ONLY RUNS ONCE
int setup(ros::NodeHandle nh_)
{
  int res = i2c_ctrl.initialize(nh_);
  if (res != 0) {
    return res;
  }
...
void loop()
{
  ...
  {
    i2c_ctrl.publishContacts(FL_sensor.isTriggered(), FR_sensor.isTriggered(), BL_sensor.isTriggered(), BR_sensor.isTriggered());

    //IMU
    if (imu_sensor.available())
    {
      geometry_msgs::Vector3 eul;
      eul.x = imu_sensor.getAngleX();
      eul.y = imu_sensor.getAngleY();
      eul.z = imu_sensor.getAngleZ();
      geometry_msgs::Vector3 acc;
      acc.x = imu_sensor.getAccX();
      acc.y = imu_sensor.getAccY();
      acc.z = imu_sensor.getAccZ();
      geometry_msgs::Vector3 gyro;
      gyro.x = imu_sensor.getGyroX();
      gyro.y = imu_sensor.getGyroY();
      gyro.z = imu_sensor.getGyroZ();

      i2c_ctrl.publishIMU(eul, acc, gyro);
    }
  ...
  // Command Servos
  if (i2c_ctrl.jointsInputIsActive())
  {
    bool step_or_view = false;
    i2c_ctrl.returnJoints(FL_, FR_, BL_, BR_, step_or_view);
  ...
  } else if (i2c_ctrl.jointsPulseIsActive())
  {
    int servo_num = i2c_ctrl.returnServoNum();
    int pulse = i2c_ctrl.returnPulse();
...
/// The Main Function ///
{
    ROS_INFO("STARTING NODE: spot_i2c");

    ros::init(argc, argv, "spot_i2c"); // register the node on ROS
    ros::NodeHandle nh_;
    last_estop = millis();
    if (0 != setup(nh_)) {
      ROS_FATAL("Failed to setup i2c devices, quitting...");
      return -1;
    }
